...
SELECT EMPNO NO, ENAME, SAL FROM EMP WHERE SAL<>3000;
    ...
    -- ex3) 부서번호 (DEPTNO)가 10번인 모든 사원의 모든 필드
    ...
    -- SQL문은 대소문자 구별이 없음. 데이터는 대소문자 구별
...
-- 4. 특정 행 출력 : WHERE절(조건절) - 논리연산자 : AND, OR, NOT
    ...
    -- ex2) 입사일(HIREDATA)이 82년도인 사원의 모든 필드
    ...
    SELECT * FROM EMP WHERE TO_CHAR (HIREDATE, 'YY/MM/DD') >='82/01/01' AND
                            TO_CHAR(HIREDATE, 'YY/MM/DD') <='82/12/31';
                            
    -- ex3) 연봉이 2400이상인 직원의 ENAME, SAL, 연봉(SAL*12)을 촐력
    SELECT ENAME, SAL, SAL*12 ANNUALSAL    -- (3) 순서
        FROM EMP                           -- (1) 순서
            WHERE SAL*12 >= 2400;          -- (2) 순서 WHERE절에는 필드의 별칭 사용 불가
            
    -- ex4) 연봉이 3000이상인 직원의 ENAME, SAL, 연봉을 연봉순으로 출력
    SELECT ENAME, SAL, SAL*12 연봉  -- (3)
        FROM EMP                         -- (1)
            WHERE SAL*12 >=3000          -- (2) WHERE절에는 필드의 별칭 사용 불가
            ORDER BY 연봉;          -- (4) ORDER BY절에는 별칭 사용 가능
...
-- 5. 산술연산자(SELECT절, 조건절, ORDER BY 절)
    -- 모든 사원의 10% 인상된 월급과 사번(EMONO), 이름(ENAME)을 출력
...
SELECT ENAME ||'은'||JOB||'업무이고 연봉은'||(SAL*12+NVL(COMM, 0))||'다' 연봉 FROM EMP;

-- 연습 문제 끝 --

-- 8. SQL연산자 (BETWEEN, IN, LIKE, IS NULL)
    -- (1) BETEWWN A AND B : A부터 B까지 (A, B포함, A<=B)
        -- EX1. SAL 이 1500이상 3000이하인 직원의 사번, 이름, 급여
        SELECT EMPNO, ENAME, SAL FROM EMP WHERE SAL>=1500 AND SAL <=3000;
        ...
        -- EX3. 이름이 'A','B','C'로 시작한ㄴ 직원의 모둔 필드
    ...
    -- (3) 필드 LIKE 패턴 : %(0글자이상), _(한글자)를 포기한 패턴
        -- EX. 이름이 M으로 시작하눈 사원의 모든 필드
    ...
    -- (4) 필드 IS NULL ( 해당 필드가 널인지 여부)
        -- EX. 상여급(COMM)을 받지 않는 사원의 모든 필드
        SELECT * FROM EMP WHERE COMM=0 OR COMM IS NULL;
        -- EX. 상여급(COMM)을 받는 사원의 모든 필드
...
-- ※ 날짜형(HIREDATE)을 문자형으로 변환 : TO_CHAR(날짜형 데이터, 패턴)
        ...
        -- HH24, HH12, HH : 시 / AM, PM / MI : 분 / SS : 초
...
SELECT * FROM EMP
    WHERE TO_CHAR(HIREDATE,'YYYY/MM/DD')LIKE'1982/__/__';
SELECT * FROM EMP WHERE TO_CHAR(HIREDATE, 'RR/MM/DD') LIKE '82%';
