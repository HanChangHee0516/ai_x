...
/* SQL
 - DCL
    사용자 계정 생성 CREATE USER, 권한 부여 GRANT, 권한박탈 REVOKE, 사용자계정삭제 DROP USER
    트랜젝션 명령어
...
CREATE TABLE BOOK(
    ...
    PRICE NUMBER(8,2), -- PRICE 필드의 타입은 숫자전체 8자리 중 소수점 2자리
...
SELECT * FROM DEPT01;
-- EMP테이블과 유사한 EMP01 테이블 : EMPNO(숫4-PX), ENAME(문10),SAL(숫7,2), DEPTNO(숫2-PK)
DROP TABLE EMP01;
DROP TABLE DEPT01;
CREATE TABLE EMP01(
...
INSERT INTO DEPT01 (DEPTNO, DNAME, LOC) VALUES ( 51, '전신','신림');
...
COMMIT; --트랜젝션 영역에 쌓여 있는 DML 명령어를 일과 실행
...
@C:\AI_X\letNote\04_DBMS\demo.sql;
...
ALTER SESSION SET NLS_DATE_FORMAT = 'YY/MM/DD';
    ...
    -- EX. SCOTT 의 부서번호를 30번으로, JOB은 'MANAGER'로 , SAL과 COMM을 500$인상, 입사일은 오늘로, MGR을 KING으로
    UPDATE EMP01
        SET DEPTNO=30,
            JOB='MANAGER',
            SAL= SAL+500,
...
SELECT ID, NAME, USERID, TO_CHAR(SALARY, '9,999,999,999.99')SALARY FROM MY_DATA;
-- 4. 트랜젝션 작업 반영
COMMIT;
-- 5. ID가 3인 사람 수정
UPDATE MY_DATA 
...
----- ★ 제약조건 : INSERT, UPDATE 할 떄 , 제약조건에 맞는 데이텀ㄴ INSERTM UPDATE됨
-- (1) PRIMARY KEY : 테이블의 각 행을 유일한 값으로 식별하기 위한 필드
-- (2) FOREIGN KEY : 테이블의 열이 다름 테이블의 열을 참조
-- (3) NOT NILL : NULL을 포함하지 않는 필드
-- (4) UNIQUE : 모든 행의 값이 유일한 값, NULL은 허용(NULL은 여러개 입력 가능)
-- (5) CHECK(조건) : 해당 조건이 만족(NILL 값 허용)
-- DEFAULT 기본값 : 해당 열의 데이터를 입력하지 않으면 원래는 NULL이 들어갈 것을 기본값이 들어가도록

-- DEPT1 & EMP1 ( EXERD를 이용하여 ERD 작성=테이블 작성 -> DDL)
...
INSERT INTO DEPT1 VALUES(20, 'IT', 'SEOUL'); -- 오류 : PK
INSERT INTO DEPT1 (DNAME, LOC) VLIES ( 'IT','SEOUL'); -- 오류 :PK
SELECT * FROM DEPT1;
INSERT INTO DEPT1 VALUES (40, 'SALES','SEOUL'); -- 오류 : UNIQUE
INSERT INTO DEPT1 (DEPTNO, DNAME) VALUES(50,'IT'); -- 오류 : NOT NULL

INSERT INTO EMP1 (EMPNO, ENAME, JOB) VALUES (1001, 'HONGGILDONGGGGGG', 'MANAGER'); -- 오류 : 길이
