...
SELECT * 
    FROM EMP, DEPT
        WHERE ENAME='SCOTT'; -- (EMP갯수) * 4(DEPT갯수) => 4행
...
SELECT EMPNO NO, ENAME, JOB, MGR, HIREDATE, SAL, COMM, E.DEPTNO, DNAME, LOC  -- (3)
    FROM EMP E, DEPT D                                                       -- (1) 
        ...
        -- EX5. JOB이 'SALESMAN'이거나 'MANAGER'인 사원의 이름, 급여, 상여, 연봉(SAL*12+COMM), 부서명, 근무지, (연봉이 큰 순) 출력
...
SELECT ENAME, JOB, SAL, DNAME
    FROM EMP E, DEPT D, SALGRADE
    WHERE E.DEPTNO=D.DEPTNO AND SAL BETWEEN LOSAL AND HISAL AND LOC='NEW YORK';
...
SELECT WORKER.EMPNO, WORKER.ENAME, WORKER.MGR, MANAGER.EMPNO, MANAGER.ENAME
    ...
    SELECT W.ENAME, M.EMPNO, M.ENAME MANAFER
    FROM EMP W, EMP M
    WHERE W.MGR=M.EMPNO;
...
-----------------------
-- ★ 4.Outer JOIN ★ -- SELF JOIN, EQUI JOIN시 조건이 만족하지 않는 행까지 나타내게 하는 JOIN
-----------------------  
-- 배제된 행을 결과에 포함시킬 경우 +기호를 정보가 부족한 컬럼이름 뒤에 덧붙임
-- (1) SELF JOUN에서의 OUTER JOIN
...
SELECT W.ENAME, W.SAL, S.GRADE, D.DNAME, M.ENAME MANAGER
    ...
    WHERE W.SAL BETWEEN S.LOSAL AND S.HISAL AND W.MGR=M.EMPNO(+) AND W.DEPTNO=D.DEPTNO AND W.SAL >= 2000;
--7. 이름, 급여, 등급, 부서명, 직속상사명, (직속상사가 없는 직원까지 전체직원 부서명 순 정렬)
SELECT W.ENAME, W.SAL, S.GRADE, D.DNAME, M.ENAME MANAGER
    FROM EMP W, EMP M, DEPT D, SALGRADE S
    WHERE W.SAL BETWEEN S.LOSAL AND S.HISAL AND W.MGR=M.EMPNO(+) AND W.DEPTNO=D.DEPTNO
    ORDER BY D.DNAME;
--8. 이름, 급여, 등급, 부서명, 연봉, 직속상사명. 연봉=(급여+comm)*12으로 계산
SELECT W.ENAME, W.SAL, GRADE, D.DNAME, (W.SAL+NVL(W.COMM,0))*12 연봉, M.ENAME MANAGER
    FROM EMP W, EMP M, DEPT D, SALGRADE S
    WHERE W.SAL BETWEEN S.LOSAL AND S.HISAL AND W.MGR=M.EMPNO(+) AND W.DEPTNO=D.DEPTNO;
...
SELECT W.ENAME, M.ENAME, K.ENAME
