-- DCL(계정생성, 권한부여, 궝한박탈, 계정삭제, 드랜젝션 명령어)
-- DDL(테이블생성, 테이블삭제, 제약조건, 시퀀스없음, 타입)
-- DML(INSERT, UPDATE, DELETE, SELECT)
	-- OUTER JOIN, AND=&&, OR=||, OCNCAT함수를 이용하여 연결연산자 대치
...
insert into major (mname, moffice) values('정통','a103호');
select * from major;
...
insert into student values (104, '이길동',4);
select * from student;
select *
...
select w.pno, w.pname, ifnull(m.pname, '★CEO★') "m.pname" 
...
-- date_format(문자, 		 포맷) => 문자형을 날짜형으로
		-- %Y(년도4자리), %y(년도2자리), %m(01,02,03,..) %c(1,2,3,,..)
		-- %d(01,02,03,..), %e(1,2,3,..), %H(24시간) %h(12시간) %p(오전,오후)  %i(분) %s(초)
